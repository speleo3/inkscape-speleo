...
def make_center(h1: HandledPoint, h2: HandledPoint) -> HandledPoint:
    (x1, y1) = h1[1]
    (x2, y2) = h2[1]
    center = [(x1 + x2) / 2, (y1 + y2) / 2]  # FIXME list vs. tuple
    return center, center, center


def reverse_csp(csp: SuperPath) -> SuperPath:
    return [[tuple(reversed(handled)) for handled in reversed(sub)]
...
class JoinPaths(Th2Effect):
    ...
    def effect(self) -> None:
        first_node: EtreeElement | None = None
        csps: list[SuperPath] = []

        for eid, node in self.selected.items():
            d = node.get("d")
            if not d:
                inkex.errormsg(f'warning: no d for id={eid}')
                continue
        ...
        first_node.attrib.pop(th2ex.sodipodi_nodetypes, None)
        first_node.attrib["d"] = cubicsuperpath.formatPath(csp)
